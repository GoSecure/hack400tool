...
package org.hackthelegacy.hack400tool.core.sql;
...
public class SqliteDbConnector {
	...
	public void insertrow(String tableName, Vector<String> values) throws SQLException {
		String queryString = "INSERT INTO " + tableName.replaceAll("[^A-Za-z0-9]", "") + " VALUES(";
		for (String value : values)
			// queryString += "\"" + String.valueOf(value).replaceAll("[^A-Za-z0-9\\-\\
			// \\.\\,\\(\\)\\[\\]\\*\\#\\_\\$\\/\\@]", "") + "\", ";
	...
	public class Query {
		...
		Query(String queryString, String[] filterParam) throws SQLException {
			...
			if (queryString.toUpperCase().startsWith("SELECT") || queryString.toUpperCase().startsWith("CREATE")
					|| queryString.toUpperCase().startsWith("INSERT") || queryString.toUpperCase().startsWith("UPDATE")
				...
				statement = dbConnection.prepareStatement("SELECT sqlcommand FROM sqlcommands WHERE id = ? LIMIT 1;");
		...
		public DefaultListModel<Object> toListModel() throws SQLException {
			// Note: The assumption is that the list elements reside in the last column

			if (resultSet == null || dbConnection == null)
				return null;

			DefaultListModel<Object> resultListModel = new DefaultListModel<Object>();
		...
		public DefaultTreeModel toTreeModel() throws SQLException {
			// Note: the table has to have first 2 rows of type integer, named (id,
			// parentid)
